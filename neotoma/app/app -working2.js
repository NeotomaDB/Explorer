                    ...
                    lang.hitch(this,function (dom, fx, domConstruct, script, Memory, array, number, aspect, registry, exExport, AllSearchResultsDialog, has, TooltipDialog, DropDownButton, popup, SpatialSearch) {
                        ...
                        try {
                            ...
                            var gphy = new OpenLayers.Layer.Google(
                                "Google Terrain",
                                { type: google.maps.MapTypeId.TERRAIN, numZoomLevels: 20, wrapDateLine: false, animationEnabled: false }
                            );

                            var ghyb = new OpenLayers.Layer.Google(
                                "Google Hybrid",
                                { type: google.maps.MapTypeId.HYBRID, numZoomLevels: 20, wrapDateLine: false, animationEnabled: false }
                            );
                            ...
                            topic.subscribe("neotoma/search/NewResult", lang.hitch(this, function () {
                                ...
                                try {
                                    ...
                                    //console.log("thisSymbol.color: " + thisSymbol.color);
                                    //thisSymbol.color = "00FF00";
                                    // get dataset count
                                    var datasetCount = 0;
                                    array.forEach(args.data,
                                        function (site) {
                                            datasetCount += site.Datasets.length;
                                        }
                                    );

                                    // add a UI for this search
                                    var newId = dojo.config.app.allSearchResults.allSearchesList.addSearch({ name: args.searchName, symbol: { color: thisSymbol.color, size: thisSymbol.size, shape: thisSymbol.shape }, request: args.request, sites: args.data, datasetCount: datasetCount });
                            ...
                            if (idsPassed === false) {
                                script.get("http://freegeoip.net/json/", { jsonp: "callback" }).then(
