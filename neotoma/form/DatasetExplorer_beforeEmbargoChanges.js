define(["dojo/_base/declare", "dijit/layout/ContentPane", "dijit/_TemplatedMixin", "dojo/text!./template/datasetExplorer.html", "dijit/_WidgetsInTemplateMixin", "dojo/request/script", "dojo/_base/lang", "dijit/Tooltip", "dgrid/OnDemandGrid", "dgrid/Grid", "neotoma/widget/_StoreMixin", "dgrid/extensions/DijitRegistry","dojo/store/Memory", "dgrid/ColumnSet", "dgrid/extensions/ColumnResizer", "dojo/dom-style", "dojo/dom-construct", "dojo/dom-class", "dojo/dom-attr", "dojo/on", "dojo/number", "dijit/registry", "dojo/dom", "dojo/has", "dojo/_base/array", "neotoma/util/export", "dojo/aspect", "dojox/embed/Flash", "dojo/_base/config", "neotoma/widget/TabContainer", "dojox/widget/Standby", "dijit/layout/BorderContainer", "dijit/form/FilteringSelect", "neotoma/form/ChartPane", "dijit/layout/StackContainer", "dijit/form/CheckBox", "dijit/form/Button", "dijit/form/Textarea", "dojo/domReady!"],
    function (declare, ContentPane, _TemplatedMixin, template, _WidgetsInTemplateMixin, script, lang, Tooltip, OnDemandGrid, Grid, StoreMixin, DijitRegistry, Memory, ColumnSet, ColumnResizer, domStyle, domConstruct, domClass, domAttr, on, numberUtil, registry, dom, has, array, exExport, aspect, Flash, config) {
        // define widget
        return declare([ContentPane, _TemplatedMixin, _WidgetsInTemplateMixin], {
                            ...
                            function (response) {
                                try {
                                    if (response.success) {
                                        ...
                                        this.site = {
                                            SiteID: currentSiteObj.SiteID || currentSiteObj.data.SiteID,
                                            SiteName: currentSiteObj.SiteName || currentSiteObj.data.SiteName,
                                            SiteDescription: currentSiteObj.SiteDescription || currentSiteObj.data.SiteDescription,
                                            Latitude: currentSiteObj.LatitudeSouth || currentSiteObj.Latitude || currentSiteObj.data.Latitude,
                                            Longitude: currentSiteObj.LongitudeEast || currentSiteObj.Longitude || currentSiteObj.data.Longitude,
                                            SiteNotes: currentSiteObj.SiteNotes || currentSiteObj.data.SiteNotes || null
                                        };
                    ...
                    script.get(config.dataServicesLocation + "/Downloads/" + datasetId,
                        ...
                        try {
                            if (response.success) {
                                ...
                                // clear previous diagram results
                                this.diagramResults = null;
                                ...
                                // load diagram, if appropriate
                                switch (datasetType) {
                                    case "pollen":
                                        // enable pollen diagram tab
                                        this.pollenDiagramTab.set("disabled", false);
                                        // set up pollen diagraming
                                        this.configurePollenDiagram(this._sheetResponse, this._datasetResponse.DefChronologyID);
                                        break;
                                    //case "vertebrate fauna":
                                    //    this.configureFaunaDiagram(this._sheetResponse);
                                    //    break;
                                    default:
                                        // disable pollen diagram tab
                                        this.pollenDiagramTab.set("disabled", true);
                                }

                                //if (datasetType === "pollen") {
                                //    // enable pollen diagram tab
                                //    this.pollenDiagramTab.set("disabled", false);
                                //    // set up pollen diagraming
                                //    this.configurePollenDiagram(this._sheetResponse);
                                ...
                                if (initRun) {
                                    ...
                                    //this.tabContainer.selectChild(this.pollenDiagramTab);
            ...
            redrawDiagram: function() {
                if (this.diagramResults) {
                    this.diagramSwf.redraw(this.diagramResults);
                }
            },
            pollenDiagramTabHidden: function () {
                if (this.diagramSwf.getResults) {
                    this.diagramResults = this.diagramSwf.getResults();
                }
            },
            pollenDiagramTabShown: function () {
                // make sure swf is loaded
                try {
                    if (this.diagramSwf === null) {
                        // initialize swf
                        this.diagramSwf = new dojox.embed.Flash({
                            path: './neotoma/swf/PollenDiagram.swf',
                            width: 800,
                            height: 400,
                            params: { wmode: 'opaque' }
                        }, this.flashPane.domNode);

                        // hook up swf methods
                        var h = aspect.after(this.diagramSwf, "onLoad", lang.hitch(this,function () {
                            try {
                                // remove aspect - only run once.
                                h.remove();

                                dojox.embed.Flash.proxy(this.diagramSwf, [
                                    "initializePollenDiagram",
                                    "getResults",
                                    "redraw"
                                ]);

                                // enable draw btn
                                this.pollenDiagramDrawBtn.set("disabled", false);

                                // draw diagram
                                //setTimeout(this.pollenDiagramDrawBtn.onClick, 1000);
                            } catch (e) {
                                alert("error in after swf onLoad: " + e.message);
                            }
                        }));
                    }
                } catch (e) {
                    alert("error in form/DatasetExplorer pollenDiagramTabShown: " + e.message);
                }
            },
            configurePollenDiagram: function(sheetResponse, defaultChronId) {
                ...
                // TODO: set y axis as default chron if available; else, default to depth
                if (!isNaN(defaultChronId)) {
            ...
            drawDiagram: function () {
                var headerRows = this._samplesHeaderGrid.get("store");
                headerRows = headerRows.data;
                var bottomRows = this._samplesGrid.get("store");
                bottomRows = bottomRows.data;
                ...
                // create combined data for now.
                var data = lang.clone(headerRows);
                data = data.concat(bottomRows);
                ...
                // draw
                this.diagramSwf.initializePollenDiagram(JSON.stringify(data), JSON.stringify(config));
            },
            postCreate: function () {
